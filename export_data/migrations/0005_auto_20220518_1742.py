# Generated by Django 3.2.10 on 2022-05-18 12:12
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('export_data', '0004_auto_20211217_1710'),
    ]

    operations = [
        migrations.RunSQL('drop function if exists replace_ids_with_text'),
...
DECLARE 
    -- Change log - v0.28
    -- a) People activity surveys - removed the people location related columns - question 1278
    -- b) corrected the logic for location hierarch update for location based activity reports - ben_type 0 
    -- c) handling check box respose with values "", "0", "123", "[123,124,152]"
    -- filename: replace_id_with_text_0.28.sql
...
BEGIN 
    ...
    if param_in_type_qid = 0 then
        select format('select concat(''response.'', a.id, case when gd_rows.id is null then '''' else ''.'' || gd_rows.id::text || ''.'' end, 
            ...
            left outer join (
                        select x1.id, x1.parent_id, x1.qtype as col_type
                        from survey_question x1
                        inner join survey_block x2 on x1.block_id = x2.id and x2.survey_id = %s
                        where parent_id is not null
                        and is_grid = false order by x2.block_order, x1.question_order
            )  as gd_cols on gd_rows.parent_id = gd_cols.parent_id 
            where (a.qtype in (''R'',''S'',''C'') or (a.qtype = ''GD'' and gd_cols.col_type in (''R'',''S'',''C'')))
            and a.active != 0
...
-- from export_csv_343_1359_temp
            ...
            EXECUTE format( 'insert into export_choice_data_temp(id,row_id, choice_id)
                            (select response_id as id, row_id, json_array_elements(replace(replace((case when "%s" in ('''',''0'') then ''[]'' when "%s" ~ ''^[1-9][0-9]*$'' then concat(''['',"%s",'']'') else "%s" end),''"'',''''),'''''''','''')::json) as choice_id 
                            from %s
                            where %s."%s" is not null
                            )', lcl_loop_row.q_id, lcl_loop_row.q_id, lcl_loop_row.q_id, lcl_loop_row.q_id, param_table_name, param_table_name, lcl_loop_row.q_id);
            EXECUTE format( 'update %s
...
if param_survey_id = 73 and param_in_type_qid = 0 then
    ...
    -- RAISE NOTICE '--------2a';
    EXECUTE format( 'update %s
        set "ai_question.1955" = coalesce(sc1955.text,''''),
        "ai_question.616" = coalesce(x6.response->>''616''::text,''''),
    ...
    left outer join survey_choice sc1955 on x6.response->>''1955''::text = sc1955.id::text
...
if param_survey_type = 0 and param_in_type_qid = 0 then
    ...
    -- beneficiary household - survey_id 70
        ...
        set 
            ...
            "ben_type_question.1955" = coalesce(sc1955.text,''''),
            ...
            "ben_type_question.619" = coalesce(sc619.text,''''),
            "ben_type_question.619.id" = (case when x4.response->>''619''::text = ''1538'' then x4.response->>''1221''::text
                                               when x4.response->>''619''::text = ''2126'' then x4.response->>''1223''::text 
                                               when x4.response->>''619''::text = ''2125'' then x4.response->>''1222''::text 
                                               else '''' end),
        ...
        left outer join survey_choice sc1955 on x4.response->>''1955''::text = sc1955.id::text
        left outer join survey_choice sc619 on x4.response->>''619''::text = sc619.id::text
    ...
    -- beneficiary Group - survey_id = 71
        ...
        EXECUTE format( 'update %s
        set "cluster.beneficiary_type_ref_name" = x3.name,
            "ben_type_question.640" = x4.response->>''640''::text,
        ...
        -- update the household beneficiary details of the people 
        EXECUTE format( 'update %s
        set "ben_type_question.1955" = coalesce(sc1955.text,''''),
            ...
            "ben_type_question.619" = coalesce(sc619.text,''''),
            "ben_type_question.619.id" = (case when x6.response->>''619''::text = ''1538'' then x6.response->>''1221''::text
                                               when x6.response->>''619''::text = ''2126'' then x6.response->>''1223''::text 
                                               when x6.response->>''619''::text = ''2125'' then x6.response->>''1222''::text 
                                               else '''' end),
            "address.1__id__" = (case when x6.response->''address''->''1''->''633''->>''1''::text ~ ''^[1-9][0-9]*$'' then (x6.response->''address''->''1''->''633''->>''1''::text)::integer else 0 end),
            "address.2__id__" = (case when x6.response->''address''->''1''->''633''->>''2''::text ~ ''^[1-9][0-9]*$'' then (x6.response->''address''->''1''->''633''->>''2''::text)::integer else 0 end),
            "address.3__id__" = (case when x6.response->''address''->''1''->''633''->>''3''::text ~ ''^[1-9][0-9]*$'' then (x6.response->''address''->''1''->''633''->>''3''::text)::integer else 0 end),
        ...
        left outer join survey_choice sc626 on x6.response->>''626''::text = sc626.id::text
        left outer join survey_choice sc619 on x6.response->>''619''::text = sc619.id::text
        left outer join survey_choice sc1955 on x6.response->>''1955''::text = sc1955.id::text
        where %s."ben_type_question.640" = x5.id::text', param_table_name,param_table_name);
...
if param_in_type_qid = 0 and param_survey_type = 1 then
    ...
    if param_ben_type = 0 then 
        ...
        lcl_boundary_level_names := Array(select lower(name) as name 
                        from masterdata_boundarylevel 
                        where code <= lcl_max_boundary_level_code order by code);
