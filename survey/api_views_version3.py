...
def new_responses_list_v3_old(request):
    ...
    if request.method == 'POST':
        ...
        if not loc_list:
            ...
            # loc_list=[]
            district_loc_list = list(UserProjectMapping.objects.filter(active=2, user_id=user_id,project__partner_mission_mapping__mission_id=2).values_list('project__district_id', flat=True).distinct())
